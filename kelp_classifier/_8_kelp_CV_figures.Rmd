...
test_set_orig <- read.csv("data/iNat_deployment_DL_list_55k.csv")
...
mmmdat_test_yrs <- mmdata(scores3, labels3,
...
mmcurves_yrs <- evalmod(mmmdat_test_yrs) 


autoplot(mmcurves_yrs, "PRC", show_cb = FALSE)


auc_ci_yrs <- auc_ci(mmcurves_yrs)

knitr::kable(auc_ci_yrs)
...
mmmdat_test_lat <- mmdata(scores2, labels2,
...
mmcurves_lat <- evalmod(mmmdat_test_lat) 

# Show average Precision-Recall curves with the 95% confidence bounds


autoplot(mmcurves_lat, "PRC", show_cb = FALSE)
