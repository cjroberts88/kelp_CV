...
conf_up = '0.52' #update cutoffs based on decision threshold 
conf_low = '0'
...
state = torch.load('Output/'+model_name+'/model_states/'+model_file+'.pt') # loads the current 'state' of my model
...
plt.savefig('Output/'+model_name+'/figures/gg_'+model_name+'_'+model_file+'_TC'+str(target_class)+'_PC'+photo_class+'_Clo'+conf_low+'_Cup'+conf_up+'.png',dpi=300, bbox_inches='tight')
